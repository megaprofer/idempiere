 ...
 *
 */
public class WListItemRendererMsgoon6 implements ListitemRenderer<Object>, EventListener<Event>, ListitemRendererExt {
	...
	private Map<Integer, Map<Integer, Object>> datos = new HashMap<Integer, Map<Integer, Object>>();

	/**
	 * Default constructor.
	 *
	 */
	public WListItemRendererMsgoon6() {
		super();
	}
	 ...
	 */
	public WListItemRendererMsgoon6(List<? extends String> columnNames) {
		super();
	...
	private void render(ListItem item, Object data, int index) {
		...
		boolean promo = false;
		try {
			color = (Boolean) row.get(row.size() - 2);
		} catch (Exception e) {
			color = false;
		}
		try {
			promo = (Boolean) row.get(row.size() - 1);
		} catch (Exception e) {
			promo = false;
		}
	 ...
	 */
	private Listcell getCellComponent(WListbox table, Object field, int rowIndex, int columnIndex) {
...
//						numberbox.setWidth("100px");
			...
			// if ID column make it invisible
				...
				org.adempiere.webui.component.Listbox cbItems = new org.adempiere.webui.component.Listbox();
				@SuppressWarnings("unchecked")
				Map<Integer, String> items = (Map<Integer, String>) field;
				cbItems.setRows(0);
				cbItems.setMultiple(false);
				cbItems.setMold("select");
				cbItems.setHflex("1");
				cbItems.removeAllItems();
				for (Integer pivote : items.keySet()) {
					KeyNamePair knp = new KeyNamePair(pivote, items.get(pivote).split(";;")[0]);
					cbItems.addItem(knp);
					if (items.get(pivote).split(";;").length > 1) {
						cbItems.setSelectedIndex(cbItems.getItems().size() - 1);
					}
				}
				if (isCellEditable) {
					cbItems.addActionListener(this);
					cbItems.addEventListener(Events.ON_CHANGE, this);
					listcell.appendChild(cbItems);
				} else {
	 ...
	 * @see #WListItemRendererMsgoon6(List)
	 */
	...
	public void onEvent(Event event) throws Exception {
		...
		} else if (source instanceof org.adempiere.webui.component.Checkbox) {
		...
		Map<Integer, Object> tmp = datos.get(row);
		tmp.put(col, value);
		datos.put(row, tmp);
		
		if(value != null && source instanceof org.adempiere.webui.component.Checkbox)
		{
	...
	public Map<Integer, Map<Integer, Object>> getDatos() {
		return datos;
	}

	public void setDatos(Map<Integer, Map<Integer, Object>> datos) {
		this.datos = datos;
	}
